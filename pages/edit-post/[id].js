...
export default function Post() {
  ...
  async function fetchPost() {
    //fetch the individual post by ipfs hash from the network
    if (!id) return;
    let provider;
    ...
    //fetching IPFS metadata from network
    ...
    //append post id to post data, we need id to make updates to the post
    data.id = postId;
    setPost(data);
  }
      ...
      {editing && (
        <div>
          ...
          <SimpleMDE
            ...
            value={post.content}
            onClick={(value) => setPost({ ...post, content: value })}
          />
